...
- **Pure reducer contract:** `advanceYear` takes only `(state, decisions[], seed)`, returns `(state, chronicle)`. No side effects, no external I/O, no RNG calls except from `rng.ts` (seeded).
